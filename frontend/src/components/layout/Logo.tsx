...
export function AuraMark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 64 64" className={className} fill="none" aria-hidden="true">
      <defs>
        <linearGradient id="auraGrad" x1="0" y1="0" x2="64" y2="64" gradientUnits="userSpaceOnUse">
          <stop stopColor="#34C88A" />
          <stop offset="1" stopColor="#0B7A5A" />
        </linearGradient>
      </defs>
      <rect width="64" height="64" rx="16" fill="url(#auraGrad)" />
      <path
...
export function Logo({ className, compact = false }: { className?: string; compact?: boolean }) {
  return (
    <div className={cn('flex items-center gap-2.5', className)}>
      <AuraMark className="h-9 w-9 shrink-0 rounded-xl shadow-soft" />
      {!compact && (
